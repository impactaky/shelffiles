{
  ...
  outputs = { self, nixpkgs, nix-ai-tools, }:
    let
      ...
      loadPackages = system: pkgs: nix-ai-tools:
        let
          packagesPath = ./packages.nix;
        in import packagesPath pkgs nix-ai-tools;
    in {
      packages = nixpkgs.lib.genAttrs systems (system: {
        default = nixpkgs.legacyPackages.${system}.buildEnv {
          name = "shelffiles-env";
          paths = loadPackages system nixpkgs.legacyPackages.${system} nix-ai-tools.packages.${system};
        };
      });
    };
}
